...
#include "RMRadialForceWaitGameplayEvent.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FApplyRootMotionRadialForceAndWaitGameplayEventDelegate, FGameplayEventData, Payload);

class AActor;

UCLASS()
class GASTOOLS_API URMRadialForceWaitGameplayEvent : public UAbilityTask_ApplyRootMotion_Base
{
	GENERATED_UCLASS_BODY()

	UPROPERTY(BlueprintAssignable)
		FApplyRootMotionRadialForceAndWaitGameplayEventDelegate OnFinish;

	UPROPERTY(BlueprintAssignable)
		FApplyRootMotionRadialForceAndWaitGameplayEventDelegate	EventReceived;

	/** Apply force to character's movement */
	UFUNCTION(BlueprintCallable, Category = "Ability|Tasks", meta = (HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
		static URMRadialForceWaitGameplayEvent* ApplyRootMotionRadialForceAndWaitGameplayEvent(UGameplayAbility* OwningAbility, FName TaskInstanceName, FVector Location, AActor* LocationActor, float Strength, float Duration, float Radius, bool bIsPush, bool bIsAdditive, bool bNoZForce, UCurveFloat* StrengthDistanceFalloff, UCurveFloat* StrengthOverTime, bool bUseFixedWorldDirection, FRotator FixedWorldDirection, ERootMotionFinishVelocityMode VelocityOnFinishMode, FVector SetVelocityOnFinish, float ClampVelocityOnFinish, FGameplayTag EventTag, AActor* OptionalExternalTarget = nullptr, bool OnlyTriggerOnce = false, bool OnlyMatchExact = true);
