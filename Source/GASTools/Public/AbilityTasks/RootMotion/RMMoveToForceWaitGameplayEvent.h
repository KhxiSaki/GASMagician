...
#include "RMMoveToForceWaitGameplayEvent.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FApplyRootMotionMoveToForceAndWaitGameplayEventDelegate, FGameplayEventData, Payload);

class AActor;
UCLASS()
class GASTOOLS_API URMMoveToForceWaitGameplayEvent : public UAbilityTask_ApplyRootMotion_Base
{
	GENERATED_UCLASS_BODY()

		UPROPERTY(BlueprintAssignable)
		FApplyRootMotionMoveToForceAndWaitGameplayEventDelegate OnTimedOut;

	UPROPERTY(BlueprintAssignable)
		FApplyRootMotionMoveToForceAndWaitGameplayEventDelegate OnTimedOutAndDestinationReached;

	UPROPERTY(BlueprintAssignable)
		FApplyRootMotionMoveToForceAndWaitGameplayEventDelegate	EventReceived;

	/** Apply force to character's movement */
	UFUNCTION(BlueprintCallable, Category = "Ability|Tasks", meta = (HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
		static URMMoveToForceWaitGameplayEvent* ApplyRootMotionMoveToForceAndWaitGameplayEvent(UGameplayAbility* OwningAbility, FName TaskInstanceName, FVector TargetLocation, float Duration, bool bSetNewMovementMode, EMovementMode MovementMode, bool bRestrictSpeedToExpected, UCurveVector* PathOffsetCurve, ERootMotionFinishVelocityMode VelocityOnFinishMode, FVector SetVelocityOnFinish, float ClampVelocityOnFinish, FGameplayTag EventTag, AActor* OptionalExternalTarget = nullptr, bool OnlyTriggerOnce = false, bool OnlyMatchExact = true);
