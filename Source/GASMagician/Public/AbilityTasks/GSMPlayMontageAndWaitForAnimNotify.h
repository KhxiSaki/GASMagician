...
#include "GSMPlayMontageAndWaitForAnimNotify.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE(FMontageWaitForAnimNotifyDelegate);
/**
 * 
 */
UCLASS()
class GASMAGICIAN_API UGSMPlayMontageAndWaitForAnimNotify : public UAbilityTask
{
GENERATED_BODY()
public:

	UGSMPlayMontageAndWaitForAnimNotify(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	
	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnCompleted;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnBlendedIn;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnBlendOut;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnInterrupted;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnCancelled;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForAnimNotifyDelegate	OnAnimNotify;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Ability|Tasks", meta = (DisplayName="PlayMontageAndWaitForAnimNotify",
		HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
	static UGSMPlayMontageAndWaitForAnimNotify* CreatePlayMontageAndWaitProxy(UGameplayAbility* OwningAbility,
