...
#include "GSMPlayMontageAndWaitForEvent.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FMontageWaitForEventDelegate, FGameplayTag, EventTag, FGameplayEventData, EventData);
/**
 * 
 */
UCLASS()
class GASMAGICIAN_API UGSMPlayMontageAndWaitForEvent : public UAbilityTask
{
	GENERATED_BODY()
public:

	UGSMPlayMontageAndWaitForEvent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	
	UPROPERTY(BlueprintAssignable)
	FMontageWaitForEventDelegate	OnCompleted;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitForEventDelegate	OnBlendedIn;
	...
	UPROPERTY(BlueprintAssignable)
	FMontageWaitForEventDelegate	OnCancelled;
	...
	static UGSMPlayMontageAndWaitForEvent* CreatePlayMontageAndWaitProxy(UGameplayAbility* OwningAbility,
